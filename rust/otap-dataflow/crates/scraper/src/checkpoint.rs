...
use crate::progress::{CheckpointBackend, CheckpointState, WriteOutcome};
...
impl CheckpointBackend for CheckpointStore {
    ...
    fn write(
        ...
        CheckpointStore::write(self, revision, cursor)
    }
}
...
impl CheckpointStore {
    ...
    fn read_from_prefix(&self, prefix: &Path) -> Result<Option<CheckpointState>, CheckpointError> {
        ...
        for entry in entries {
            ...
            } else if name.starts_with(&format!("{prefix_name}.")) && name.ends_with(".json") {
