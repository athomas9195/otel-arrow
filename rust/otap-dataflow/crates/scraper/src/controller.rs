...
/// Quarantines ownership until process exit if an offloaded operation cannot
/// be joined. Releasing it would allow a replacement to overlap native work.
...
impl<L: SourceOwnership> HeldOwnership<L> {
...
impl<L> Drop for HeldOwnership<L> {
    fn drop(&mut self) {
        if self.abandoned.get() || !self.cleanup_joined.get() {
            // Intentional fail-closed quarantine, not a reusable retry path.
            // OS locks are released only by process termination.
            std::mem::forget(self.lease.take());
            otel_warn!(
                "database_receiver.worker_abandoned",
                message = "Worker missed its stop deadline; source ownership quarantined until process restart"
            );
        }
    }
}
...
impl<A, S, L> DatabaseReceiver<A, S, L>
where
    A: DriverAdapter,
    S: CheckpointBackend,
    L: SourceOwnership,
...
fn ensure_cursor_advanced(
    ...
    fn timestamp(value: &str) -> Result<chrono::DateTime<chrono::Utc>, ProgressError> {
        chrono::DateTime::parse_from_rfc3339(value)
            ...
            .map_err(|_| ProgressError::InvalidTimestamp)
    }
    if (timestamp(&candidate.timestamp)?, candidate.tie_breaker)
        <= (timestamp(&committed.timestamp)?, committed.tie_breaker)
    {
...
#[async_trait(?Send)]
impl<A, S, L> local::Receiver<OtapPdata> for DatabaseReceiver<A, S, L>
where
    A: DriverAdapter + 'static,
    S: CheckpointBackend,
    L: SourceOwnership + 'static,
{
    async fn start(
        self: Box<Self>,
...
async fn read_checkpoint<S: CheckpointBackend>(
    store: &S,
    effect_handler: &local::EffectHandler<OtapPdata>,
...
async fn commit_checkpoint<S: CheckpointBackend>(
    store: &S,
    revision: u64,
    candidate: &CompositeCursor,
    max_failures: u32,
...
#[cfg(test)]
#[path = "checkpoint_control_tests.rs"]
mod checkpoint_control_tests;
