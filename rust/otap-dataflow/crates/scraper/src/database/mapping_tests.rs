...
/// Scenario: Ten thousand rows each contain a 4 KiB text value.
/// Guarantees: The maximum row-count page encodes off-core while the local runtime remains runnable.
#[tokio::test]
async fn large_owned_page_keeps_local_runtime_responsive() {
    let work = tokio::task::spawn_blocking(|| {
        let rows = (0..10_000)
            ...
            .collect();
        encode(
            page(vec![column("PAYLOAD", "VARCHAR2")], rows),
    ...
    let encoded = work.await.expect("encoding worker");
    assert_eq!(encoded.row_count, 10_000);
    assert!(encoded.encoded_bytes < UNLIMITED_BYTES as usize);
    assert!(ticks > 0);
}
