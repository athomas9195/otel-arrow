...
/// Scenario: The native worker ignores cancellation past an already-expired stop deadline.
...
fn quarantined_ownership_is_retained() {
    ...
    drop(HeldOwnership {
        lease: Some(Lease(Rc::clone(&dropped))),
...
/// Guarantees: Abnormal task cancellation cannot release ownership before worker cleanup joins.
#[tokio::test]
async fn dropped_receiver_future_quarantines_ownership() {
    ...
    let dropped = Rc::new(Cell::new(false));
    let lease = Lease(Rc::clone(&dropped));
...
/// Scenario: Cursor timestamps use different UTC offsets or fractional precision.
/// Guarantees: Advancement compares instants, not lexicographic timestamp spellings.
#[test]
fn cursor_ordering_uses_utc_instants() {
    assert!(
        ensure_cursor_advanced(
            &CompositeCursor::new("2026-01-01 10:00:00 +00:00".into(), 1),
            &CompositeCursor::new("2026-01-01 11:00:00 +02:00".into(), 2),
        )
        .is_err()
    );
    assert!(
        ensure_cursor_advanced(
            &CompositeCursor::new("2026-01-01 10:00:00.0 +00:00".into(), 1),
            &CompositeCursor::new("2026-01-01 10:00:00 +00:00".into(), 2),
        )
        .is_ok()
    );
}
...
// Shared with the controller's blocking persistence workers. This backend is
// test-only and verifies the contract without depending on filesystem storage.
#[derive(Clone, Default)]
struct MemoryCheckpoint(Arc<Mutex<Option<CheckpointState>>>);

#[derive(Debug, thiserror::Error)]
#[error("test checkpoint revision mismatch or overflow")]
struct TestCheckpointError;

impl CheckpointBackend for MemoryCheckpoint {
    type Error = TestCheckpointError;

    fn read(&self) -> Result<Option<CheckpointState>, Self::Error> {
        Ok(self.0.lock().expect("test checkpoint mutex").clone())
    }
    ...
    fn write(
        ...
        let mut stored = self.0.lock().expect("test checkpoint mutex");
        if stored.as_ref().map_or(0, |state| state.revision) != revision {
            return Err(TestCheckpointError);
        }
        let state = CheckpointState {
            revision: revision.checked_add(1).ok_or(TestCheckpointError)?,
            cursor: cursor.clone(),
        };
...
impl SourceOwnership for TestOwnership {
...
/// Guarantees: The production loop writes the last emitted cursor through its checkpoint
/// contract before processing shutdown, without requiring a concrete filesystem backend.
#[test]
fn matching_ack_commits_through_the_checkpoint_contract() {
    let checkpoint = CheckpointConfig {
        directory: "test-only-memory-checkpoint".to_owned(),
        on_nack: OnNack::Rewind,
        nack_backoff: Duration::from_millis(10),
        max_consecutive_failures: 3,
    };
    let store = MemoryCheckpoint::default();
