...
pub const MYSQL_RECEIVER_URN: &str = "urn:otel:receiver:mysql";

#[allow(unsafe_code)]
#[otel_arrow_dfe_engine::component_inventory(category = Receiver)]
#[linkme::distributed_slice(OTAP_RECEIVER_FACTORIES)]
pub static MYSQL_RECEIVER: ReceiverFactory<OtapPdata> = ReceiverFactory {
    name: MYSQL_RECEIVER_URN,
    create: create_mysql_receiver,
    validate_config: validate_mysql_config,
    wiring_contract: otel_arrow_dfe_engine::wiring_contract::WiringContract::UNRESTRICTED,
};

fn create_mysql_receiver(
    ...
    todo!("bind MySqlScraper to the shared DatabaseReceiver")
}

fn validate_mysql_config(_config: &Value) -> Result<(), Error> {
    todo!("validate shared and MySQL-specific configuration")
}
